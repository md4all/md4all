# Adapted from https://github.com/seawee1/ForkGAN-pytorch/blob/master/models/base_model.py
import os
import torch
...
class BaseModel(ABC):
    ...
    def load_networks(self, epoch):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                if not os.path.isfile(load_path):
                    print('%s not found... skipping...' % load_path)
                    continue
                net = getattr(self, 'net' + name)
                print('loading the model from %s' % load_path)
                # if you are using PyTorch newer than 0.4 (e.g., built from
                # GitHub source), you can remove str() on self.device
                state_dict = torch.load(load_path, map_location=str('cuda:' + str(self.gpu)))
        ...
        if hasattr(self, 'scaler'):
            scaler = getattr(self, 'scaler')
            load_filename = '%s_scaler.pth' % (epoch)
            load_path = os.path.join(self.save_dir, load_filename)
            if not os.path.isfile(load_path):
                print('%s not found... skipping...' % load_path)
            else:
