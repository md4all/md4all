...
def transformation_from_parameters(pose_vec, invert=False):
    """Convert the network's (axisangle, translation) output into a 4x4 matrix
    """
    trans, rot = pose_vec[:, :3].unsqueeze(1), pose_vec[:, 3:].unsqueeze(1)
    R = rot_from_axisangle(rot)
    t = trans.clone()
...
def upsample(x, mode='nearest'):#bilinear
